...
class NamedArgumentsRuleTest : Spek({

    describe("named-arguments rule") {
        ...
        val rule = NamedArgumentsRule()

        it("test for Named Arguments Rule") {
            assertThat(rule.lint("""
fun add(x: Int, y: Int): Int { return x + y }
fun fn() { add(x=10, 20) }
            """.trimIndent()))
                .isEqualTo(listOf(LintError(2, 22, "named-arguments", "found a function with no named arguments")))
        }
    }
})
